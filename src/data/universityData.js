export const universityData = [
    {
        id: 1,
        name: "COLLEGE OF SCIENCE AND ENGINEERING",
        departments: [
          { 
            id: 1, 
            title: "BACHELOR DEGREE PROGRAM OF DATA SCIENCE", 
            programs: [{ id: 1, name: "BACHELOR" }] 
          },
          { 
            id: 2, 
            title: "DEPARTMENT OF APPLIED MATHEMATICS", 
            programs: [
              { id: 1, name: "DIVISION OF MATHEMATICAL SCIENCE" },
              { id: 2, name: "DIVISION OF STATISTICAL SCIENCE" },
              { id: 3, name: "MASTER" },
              { id: 4, name: "MASTER PROGRAM OF STATISTICS" },
              { id: 5, name: "Ph.D." }
            ] 
          },
          { 
            id: 3, 
            title: "DEPARTMENT OF CHEMISTRY", 
            programs: [
              { id: 1, name: "BACHELOR" },
              { id: 2, name: "MASTER PROGRAM OF CHEMISTRY" },
              { id: 3, name: "MASTER PROGRAM OF CHEMISTRY (INTERNATIONAL)" },
              { id: 4, name: "PH.D. PROGRAM IN CHEMISTRY (GENERAL)" },
              { id: 5, name: "PH.D. PROGRAM IN CHEMISTRY (INTERNATIONAL)" }
            ] 
          },
          { 
            id: 4, 
            title: "DEPARTMENT OF PHYSICS", 
            programs: [
              { id: 1, name: "DIVISION OF PHYSICS" },
              { id: 2, name: "DIVISION OF NANO AND PHOTOELECTRIC SCIENCE" },
              { id: 3, name: "MASTER PROGRAM OF APPLIED PHYSICS" },
              { id: 4, name: "MASTER PROGRAM OF APPLIED PHYSICS (INTERNATIONAL)" },
              { id: 5, name: "PH.D. PROGRAM IN APPLIED PHYSICS" },
              { id: 6, name: "PH.D. PROGRAM IN PHYSICS (INTERNATIONAL)" }
            ] 
          },
          { 
            id: 5, 
            title: "DEPARTMENT OF COMPUTER SCIENCE AND INFORMATION ENGINEERING", 
            programs: [
              { id: 1, name: "BACHELOR PROGRAM OF COMPUTER SCIENCE AND INFORMATION ENGINEERING" },
              { id: 2, name: "BACHELOR PROGRAM OF COMPUTER SCIENCE AND INFORMATION ENGINEERING (INTERNATIONAL)" },
              { id: 3, name: "MASTER" },
              { id: 4, name: "MASTER PROGRAM OF COMPUTER SCIENCE AND INFORMATION ENGINEERING" },
              { id: 5, name: "MASTER PROGRAM OF COMPUTER SCIENCE AND INFORMATION ENGINEERING (INTERNATIONAL)" },
              { id: 6, name: "MASTER PROGRAM IN ARTIFICIAL INTELLIGENCE AND INNOVATIVE APPLICATIONS" },
              { id: 7, name: "(in-service master program) EXTENSIVE MASTER PROGRAM OF COMPUTER SCIENCE AND INFORMATION ENGINEERING (EXTENSIVE)" },
              { id: 8, name: "Ph.D." }
            ] 
          },
          { 
            id: 6, 
            title: "DEPARTMENT OF MATERIALS SCIENCE AND ENGINEERING", 
            programs: [
              { id: 1, name: "BACHELOR" },
              { id: 2, name: "MASTER PROGRAM OF MATERIALS SCIENCE AND ENGINEERING" },
              { id: 3, name: "MASTER PROGRAM OF MATERIALS SCIENCE AND ENGINEERING (INTERNATIONAL)" },
              { id: 4, name: "PH.D. PROGRAM IN MATERIALS SCIENCE AND ENGINEERING" },
              { id: 5, name: "PH.D. PROGRAM IN MATERIALS SCIENCE AND ENGINEERING (INTERNATIONAL)" }
            ] 
          },
          { 
            id: 7, 
            title: "DEPARTMENT OF ELECTRICAL ENGINEERING", 
            programs: [
              { id: 1, name: "BACHELOR" },
              { id: 2, name: "MASTER" },
              { id: 3, name: "Ph.D." }
            ] 
          },
          { 
            id: 8, 
            title: "DEPARTMENT OF OPTO-ELECTRONIC ENGINEERING", 
            programs: [
              { id: 1, name: "BACHELOR" },
              { id: 2, name: "MASTER" }
            ] 
          },
          { 
            id: 9, 
            title: "DEPARTMENT OF BIOCHEMISTRY AND MOLECULAR MEDICINE", 
            programs: [
              { id: 1, name: "BACHELOR" },
              { id: 2, name: "MASTER" },
              { id: 3, name: "PH.D." }
            ] 
          },
          { 
            id: 10, 
            title: "BACHELOR", 
            programs: []
          }
        ]
      },
      {
        id: 2,
        name: 'COLLEGE OF MANAGEMENT',
        departments: [
            { 
                id: 1, 
                title: 'EXECUTIVE MASTER PROGRAM OF BUSINESS ADMINISTRATION (in-service master program)', 
                programs: [
                    { id: 1, name: '(in-service master program) SENIOR MANAGEMENT' },
                    { id: 2, name: '(in-service master program) INNOVATION AND ENTREPRENEURSHIP MANAGEMENT' }
                ]
            },
            { 
                id: 2, 
                title: 'BACHELOR PROGRAM OF MANAGEMENT SCIENCE AND FINANCE (international program)', 
                programs: [{ id: 1, name: 'BACHELOR' }]
            },
            { 
                id: 3, 
                title: 'BACHELOR PROGRAM OF ACCOUNTING AND INFORMATION MANAGEMENT (international program)', 
                programs: [{ id: 1, name: 'BACHELOR' }]
            },
            { 
                id: 4, 
                title: 'BACHELOR PROGRAM OF DIGITAL MARKETING AND SERVICE INNOVATION', 
                programs: [{ id: 1, name: 'BACHELOR' }]
            },
            { 
                id: 5, 
                title: 'DEPARTMENT OF BUSINESS ADMINISTRATION', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER PROGRAM OF BUSINESS ADMINISTRATION' },
                    { id: 3, name: 'MASTER PROGRAM OF BUSINESS ADMINISTRATION (INTERNATIONAL)' },
                    { id: 4, name: '(in-service master program) EXTENSIVE MASTER PROGRAM OF BUSINESS ADMINISTRATION' },
                    { id: 5, name: 'Ph.D.' }
                ]
            },
            { 
                id: 6, 
                title: 'DEPARTMENT OF INTERNATIONAL BUSINESS', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' },
                    { id: 3, name: '(in-service master program) EXECUTIVE BUSINESS ADMINISTRATION MASTER PROGRAM OF INTERNATIONAL BUSINESS' }
                ]
            },
            { 
                id: 7, 
                title: 'DEPARTMENT OF ACCOUNTING', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' }
                ]
            },
            { 
                id: 8, 
                title: 'DEPARTMENT OF INFORMATION MANAGEMENT', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER PROGRAM OF INFORMATION MANAGEMENT' },
                    { id: 3, name: 'MASTER PROGRAM OF INFORMATION MANAGEMENT' }
                ]
            },
            { 
                id: 9, 
                title: 'DEPARTMENT OF FINANCE', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' }
                ]
            },
            { 
                id: 10, 
                title: 'GRADUATE INSTITUTE OF LOGISTICS MANAGEMENT', 
                programs: [
                    { id: 1, name: 'MASTER' }
                ]
            },
            { 
                id: 11, 
                title: 'DEPARTMENT OF TOURISM, RECREATION AND LEISURE STUDIES', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' }
                ]
            },
            { 
                id: 12, 
                title: 'BACHELOR', 
                programs: []
            }
        ]
    },
    {
        id: 3,
        name: 'COLLEGE OF HUMANITIES AND SOCIAL SCIENCES',
        departments: [
            { 
                id: 1, 
                title: 'INTERNATIONAL PH.D. PROGRAM IN TEACHING CHINESE AS A SECOND LANGUAGE', 
                programs: [{ id: 1, name: 'PH.D.' }]
            },
            { 
                id: 2, 
                title: 'INTERNATIONAL MASTER PROGRAM IN TEACHING CHINESE AND CALLIGRAPHY', 
                programs: [{ id: 1, name: 'MASTER' }]
            },
            { 
                id: 3, 
                title: 'PH.D. PROGRAM IN ASIA-PACIFIC REGIONAL STUDIES', 
                programs: [{ id: 1, name: 'PH.D.' }]
            },
            { 
                id: 4, 
                title: 'DEPARTMENT OF SINOPHONE LITERATURES', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' },
                    { id: 3, name: 'DIVISION OF CREATIVE WRITING' },
                    { id: 4, name: 'DIVISION OF LITERATURE AND CULTURAL STUDIES' }
                ]
            },
            { 
                id: 5, 
                title: 'DEPARTMENT OF ENGLISH', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'DIVISION OF TEACHING ENGLISH TO SPEAKERS OF OTHER LANGUAGES' }
                ]
            },
            { 
                id: 6, 
                title: 'DEPARTMENT OF HISTORY', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' }
                ]
            },
            { 
                id: 7, 
                title: 'DEPARTMENT OF COUNSELING AND CLINICAL PSYCHOLOGY', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' }
                ]
            },
            { 
                id: 8, 
                title: 'DEPARTMENT OF ECONOMICS', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' },
                    { id: 3, name: 'Ph.D.' }
                ]
            },
            { 
                id: 9, 
                title: 'DEPARTMENT OF CHINESE LANGUAGE AND LITERATURE', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' },
                    { id: 3, name: 'Ph.D.' }
                ]
            },
            { 
                id: 10, 
                title: 'DEPARTMENT OF TAIWAN AND REGIONAL STUDIES', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' },
                    { id: 3, name: '(in-service master program) EXTENSIVE MASTER PROGRAM OF TAIWAN AND REGIONAL STUDIES' }
                ]
            },
            { 
                id: 11, 
                title: 'DEPARTMENT OF SOCIOLOGY', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' }
                ]
            },
            { 
                id: 12, 
                title: 'DEPARTMENT OF PUBLIC ADMINISTRATION', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' },
                    { id: 3, name: '(in-service master program) EXTENSIVE MASTER PROGRAM OF PUBLIC ADMINISTRATION' }
                ]
            },
            { 
                id: 13, 
                title: 'DEPARTMENT OF LAW', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' }
                ]
            },
            { 
                id: 14, 
                title: 'DEPARTMENT OF LAW (indigenous law program)', 
                programs: [{ id: 1, name: 'BACHELOR' }]
            },
            { 
                id: 15, 
                title: 'BACHELOR', 
                programs: []
            }
        ]
    },
    {
        id: 4,
        name: 'COLLEGE OF INDIGENOUS STUDIES',
        departments: [
            { 
                id: 1, 
                title: 'INTERNATIONAL PH. D. PROGRAM IN INDIGENOUS STUDIES', 
                programs: [{ id: 1, name: 'PH.D.' }]
            },
            { 
                id: 2, 
                title: 'DEPARTMENT OF ETHNIC RELATIONS AND CULTURES', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' },
                    { id: 3, name: '(in-service master program) EXTENSIVE MASTER PROGRAM OF ETHNIC RELATIONS AND CULTURES' },
                    { id: 4, name: 'Ph.D.' }
                ]
            },
            { 
                id: 3, 
                title: 'DEPARTMENT OF INDIGENOUS LANGUAGES AND COMMUNICATION', 
                programs: [{ id: 1, name: 'BACHELOR' }]
            },
            { 
                id: 4, 
                title: 'UNDERGRADUATE PROGRAM OF INDIGENOUS PERFORMANCE AND ARTS', 
                programs: [{ id: 1, name: 'BACHELOR' }]
            },
            { 
                id: 5, 
                title: 'DEPARTMENT OF INDIGENOUS DEVELOPMENT AND SOCIAL WORK', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'DIVISION OF INDIGENOUS DEVELOPMENT' },
                    { id: 3, name: 'DIVISION OF SOCIAL WORK' },
                    { id: 4, name: 'MASTER' },
                    { id: 5, name: '(in-service master program) EXTENSIVE MASTER PROGRAM OF INDIGENOUS SOCIAL WORK' }
                ]
            },
            { 
                id: 6, 
                title: 'BACHELOR', 
                programs: []
            }
        ]
    },
    {
        id: 5,
        name: 'COLLEGE OF THE ARTS',
        departments: [
            { 
                id: 1, 
                title: 'THE PH.D. PROGRAM IN INTERDISCIPLINARY ARTS', 
                programs: [{ id: 1, name: 'PH.D.' }]
            },
            { 
                id: 2, 
                title: 'DEPARTMENT OF ARTS AND CREATIVE INDUSTRIES', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' },
                    { id: 3, name: '(in-service master program) EXECUTIVE MASTER PROGRAM OF ARTS AND CREATIVE INDUSTRIES' }
                ]
            },
            { 
                id: 3, 
                title: 'DEPARTMENT OF MUSIC', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' }
                ]
            },
            { 
                id: 4, 
                title: 'DEPARTMENT OF ARTS AND DESIGN', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' }
                ]
            },
            { 
                id: 5, 
                title: 'BACHELOR', 
                programs: []
            }
        ]
    },
    {
        id: 6,
        name: 'HUA-SHIH COLLEGE OF EDUCATION',
        departments: [
            { 
                id: 1, 
                title: 'MASTER PROGRAM OF MULTICULTURAL EDUCATION', 
                programs: [{ id: 1, name: 'MASTER' }]
            },
            { 
                id: 2, 
                title: 'PH.D. PROGRAM OF MULTICULTURAL EDUCATION', 
                programs: [{ id: 1, name: 'PH.D.' }]
            },
            { 
                id: 3, 
                title: 'DEPARTMENT OF EARLY CHILDHOOD EDUCATION', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'POST-BACCALAUREATE PROGRAM IN EDUCARE GIVER' },
                    { id: 3, name: 'MASTER' },
                    { id: 4, name: '(in-service master program) EXTENSIVE MASTER PROGRAM OF EARLY CHILDHOOD EDUCATION' }
                ]
            },
            { 
                id: 4, 
                title: 'DEPARTMENT OF SPECIAL EDUCATION', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER PROGRAM OF INDIVIDUALS WITH SPECIAL NEEDS AND ASSISTIVE TECHNOLOGY' }
                ]
            },
            { 
                id: 5, 
                title: 'DEPARTMENT OF EDUCATIONAL ADMINISTRATION AND MANAGEMENT', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' },
                    { id: 3, name: 'EXTENSIVE MASTER PROGRAM OF SCHOOL ADMINISTRATION' },
                    { id: 4, name: '(in-service master program) EXTENSIVE MASTER PROGRAM OF EDUCATION ADMINISTRATION AND MANAGEMENT' }
                ]
            },
            { 
                id: 6, 
                title: 'DEPARTMENT OF EDUCATION AND HUMAN POTENTIALS DEVELOPMENT', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER PROGRAM OF EDUCATION' },
                    { id: 3, name: '(in-service master program) EXTENSIVE MASTER PROGRAM OF EDUCATION' },
                    { id: 4, name: 'MASTER PROGRAM OF SCIENCE EDUCATION' },
                    { id: 5, name: 'PH.D. PROGRAM OF EDUCATION' },
                    { id: 6, name: 'PH.D. PROGRAM OF SCIENCE EDUCATION' }
                ]
            },
            { 
                id: 7, 
                title: 'DEPARTMENT OF PHYSICAL EDUCATION AND KINESIOLOGY', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' },
                    { id: 3, name: 'EXTENSIVE MASTER PROGRAM OF PHYSICAL EDUCATION AND KINESIOLOGY' },
                    { id: 4, name: '(in-service master program) EXTENSIVE MASTER PROGRAM OF PHYSICAL EDUCATION AND KINESIOLOGY' }
                ]
            },
            { 
                id: 8, 
                title: 'BACHELOR', 
                programs: []
            }
        ]
    },
    {
        id: 7,
        name: 'ENTER FOR TEACHER EDUCATION',
        departments: [
            { id: 1, title: 'EDUCATION PROGRAM FOR ELEMENTARY TEACHER EDUCATION' },
            { id: 2, title: 'EDUCATION PROGRAM FOR SECONDARY TEACHER EDUCATION' },
            { id: 3, title: 'THE PROGRAM FOR PROMOTING SPECIALTIES OF PROMINENT TEACHER-TO-BE OF NDHU' },
            { id: 4, title: 'COURSE OF BILLINGUAL TEACHER EDUCATION' }
        ]
    },
    {
        id: 8,
        name: 'COLLEGE OF ENVIRONMENTAL STUDIES AND OCEANOGRAPHY',
        departments: [
            { 
                id: 1, 
                title: 'DEPARTMENT OF NATURAL RESOURCES AND ENVIRONMENTAL STUDIES', 
                programs: [
                    { id: 1, name: 'BACHELOR' },
                    { id: 2, name: 'MASTER' },
                    { id: 3, name: 'PH.D.' }
                ]
            },
            { 
                id: 2, 
                title: 'GRADUATE INSTITUTE OF MARINE BIOLOGY', 
                programs: [
                    { id: 1, name: 'MASTER' },
                    { id: 2, name: 'PH.D.' }
                ]
            },
            { 
                id: 3, 
                title: 'MASTER OF HUMANITY AND ENVIRONMENTAL SCIENCE PROGRAM', 
                programs: [
                    { id: 1, name: 'MASTER' }
                ]
            },
            { 
                id: 4, 
                title: 'BACHELOR', 
                programs: []
            }
        ]
    },

  ];
  
